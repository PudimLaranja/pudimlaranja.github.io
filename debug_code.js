...
class Cube {
    ...
    __SetRotation__(rotation) {
        this.points.forEach((point, idx) => {
            let vec = this.shape[idx];
            let rotVecX = new Vector2(vec.x, vec.y).rotate(rotation.x * Math.PI / 180); // Convert degrees to radians
            vec = new Vector3(rotVecX.x, rotVecX.y, vec.z);
            let rotVecY = new Vector2(vec.x, vec.z).rotate(rotation.y * Math.PI / 180); // Corrected typo and conversion
            vec = new Vector3(rotVecY.x, vec.y, rotVecY.y);
            let rotVecZ = new Vector2(vec.z, vec.y).rotate(rotation.z * Math.PI / 180); // Ensure Z rotation is also in radians
...
let cube = new Cube("blue");

cube.rotation.y = PI2 / 2;
